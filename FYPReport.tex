...
This search tree is of depth 2, with 2 partitions on the left side for elements less than Pivot 2 and vice versa. Otherwise, if we use a unbalanced BST that puts Pivot 1 or Pivot 3 on top, we would have a tree of depth 3 and is not efficient enough.
The entire BST would look like a \^{} symbol, we just need to extend the length of its two 'wings' as pivots increase.
Bearing this layout in mind, it is not hard to tell that we are going to have 9 branches for 5-Pivot QuickSort (3 partitions on each side, $3 \times 3 = 9$) and 12 branches for 6-Pivot QuickSort (3 partitions on one side and 4 on the other, $3 \times 4 = 12$).
The complexity of the partitioning method is growing way too far and thus only 4-Pivot QuickSort will be discussed in this thesis. In the \cite{HowGoodIsMultiPivotQuicksort}{'how good is multi-pivot quicksort'} paper, the author sketched an extremal tree for seven pivots in fig 10 of his thesis.
