...
pub unsafe fn rotate_$n<T>(arr: &mut [T], idx: [usize; $n]) {
    ...
    let tmp = ptr::read(&arr[idx[0]]);
    for i in 1..$n {
        ptr::copy_nonoverlapping(arr.get_unchecked(idx[i]),
            arr.get_unchecked_mut(idx[i - 1]), 1);
    }
    ptr::copy_nonoverlapping(&tmp,
        arr.get_unchecked_mut(idx[n - 1]), 1);
}
\end{verbatim}

Using direct pointer manipulation and $get_unchecked$ function, we can avoid the overhead of the Rust borrow checker and array boundary checking to maximize the performance.
