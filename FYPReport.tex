...
The method shows no difference in terms of both comparison and swaps with the original Dual-Pivot Quicksort, but it turned out that it is as fast as the Yaroslavskiy's partition, and even faster than another method along, named 'Countering Strategy C', who claimed to have the least 
...
with 3 seconds of warm-up time and 5 seconds of measurement time for each test. The results are calculated over regression tests with 100, 1000, 10000, 100000, 1000000 and 10000000 elements,
...
But things go beyond our expectations, the 4-Pivot partition shows better statistics of branch misses than the 3-Pivot partition method, which is a surprise to us,
...
the possible instabilities of the 4-Pivot partition method. It is still worth our notice that it happens when more pivots could reduce the branch misses.

Apart from the branch misses, the cache misses are also a significant factor that could affect a lot, which is what we are going to shift our focus to.
