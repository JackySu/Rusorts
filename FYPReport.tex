...
Thankfully, for \hyperlink{fig:4pivot}{4-Pivot QuickSort}, \hypertarget{2MoreBranches}we are just adding two more branches (As showed in the blue-red and purple-red branches in the chart below)
...
Zooming in, the traditional means exhibit minimal variances between the 1-Pivot Hoare and Lomuto partition methods,
it makes sense that the 1-Pivot Hoare's partition method tend to always have a slightly higher mean of branch misses than the 1-Pivot Lomuto's partition method, due to its need to locate and swap 2 out-of-order elements, while only 1 for Lomuto's.
We are bound to have some more branch misses in the former case.
But the 2-Pivot Yaro's partition method presents a slightly higher mean of branch misses, likely owing to the introduction of additional branches with the second pivot.
While the complexities scale linearly with the number of pivots, the standard deviation grows rapidly as the size of array exceeds 10k, and explodes after 1 million.
...
Among all variations, the 2-Pivot Yaroslavskiy's partition method consistently hold the lowest mean of both cache misses and branch misses combined together in most of the larger array tests, with stable standard deviations.
...
\cite{Kushagra}, the tests conducted in this thesis show that the cache misses do not consistently decrease with more pivots. For instance, the 3-Pivot Kushagra's method shows more cache misses than the 2-Pivot Yaroslavskiy's,
...
It brings to our attention that the testing CPU, R7 3700x, features a 16MB L3 cache, which is divided into two 8MB segments per CCD ($2 CCD \times 8MB = 16MB$), with each segment serving 4 cores.
...
\begin{tabular}{ |c c | c | }
    ...
    \hline
    2-Pivot Yaro    & 100000   & 1200.81      \\
                    & 1000000  & 22058.59     \\
                    & 10000000 & 598454.20    \\

    \hline
    3-Pivot Kush    & 100000   & 1071.02      \\
                    & 1000000  & 22472.11     \\
                    & 10000000 & 608203.30    \\
    \hline
    4-Pivot         & 100000   & 1158.25      \\
                    & 1000000  & 22077.32     \\
                    & 10000000 & 587479.30    \\
    \hline
\end{tabular}
...
In conclusion, our experiments reveal unexpected nuances in algorithm performance comparisons, the 2-Pivot Yaroslavskiy's method has lower cache misses than 3-Pivot, least branch misses but deficient performance on $10^7$ size of data, the 4-Pivot method has the least amount of time,
somewhat partially due to its favorable cache behavior, but more importantly, less branch misses in field experiments than predicted. The discrepancy between predicted and actual branch misses in the 4-Pivot method suggests new avenues for exploration and optimization.
