...
\begin{algorithm}[H]
    \small
	\caption{Block partitioning}\label{BlockPartition}
	\begin{algorithmic}[1]
		\Procedure{BlockPartition}{$A[\ell,\dots, r]$, pivot}
		...
		\For{$i = 0,\dots, \bs - 1$} 
		\State $\mathrm{offsets}_L[\mathrm{num}_L ] \gets i$\Comment{Will this regardless write slows it down?}
		\State $\mathrm{num}_L \pluseq (\mathrm{pivot} \geq A[\ell + i] )$\Comment{scanning phase for left side} 
		\EndFor
		\EndIf
		...
		\State $\mathrm{num}_R \pluseq (\mathrm{pivot} \leq A[r - i] )$ \Comment{scanning phase for right side}
		\EndFor	
		\EndIf
...
It is often believed that Hoare's partition method is more efficient than the Lomuto's, with the support of fewer swaps, partitioning efficiency and memory efficiency.
% TODO: finish
...
% TODO: 29th page of 'how good is multi-pivot quicksort' discusses that the write-back is supported by modern cpus that has much less cost than write-through,
% which explains why keep writing to the same cache line is not a big deal for blockquicksort when dealing with storing offsets into the 128 size blocks.
